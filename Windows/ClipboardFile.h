...
class CClipboardFile {
	...
	BOOL WriteFile() {
		...
		if (file.Create(m_strFilePath)) {
			...
			ATLVERIFY(file.Write(ClipboardFileTextPrefix, (DWORD) wcslen(ClipboardFileTextPrefix) * sizeof WCHAR));
			if (m_wstrContent.IsEmpty())
				ATLVERIFY(file.SetEOF());
			else
				ATLVERIFY(file.Write((LPCWSTR) m_wstrContent, m_wstrContent.GetLength() * sizeof WCHAR));
			ATLVERIFY(file.SetEOF());
			return TRUE;
		}
		return FALSE;
