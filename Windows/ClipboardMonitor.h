...
public:
	...
	CClipboardMonitor() {
      m_uClipboardChangedMessage = ::RegisterWindowMessage(TEXT("ClipboardMonitor_ClipboardChangedMessage"));
	...
	LRESULT OnDrawCliboard(UINT uMsg,
		...
		LPARAM lParam,
		BOOL& /*bHandled*/) {
		SendMessage(m_hwndClipboardViewer, m_uClipboardChangedMessage, 0, (LPARAM) this);
		if (m_hwndNextClipboardViewer != NULL)
			SendMessage(m_hwndNextClipboardViewer, uMsg, wParam, lParam); 
		return 0;
	}
