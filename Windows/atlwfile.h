...
public:
    ...
    BOOL Write( LPCVOID lpBuf, DWORD nCount ) {
        ...
        if( !::WriteFile( m_hFile, lpBuf, nCount, &dwBytesWritten,
                          NULL ) ) return FALSE;
        return TRUE;
    }
    ...
    BOOL GetFileTime( FILETIME * ftCreate, FILETIME * ftAccess,
                      FILETIME * ftModified ) {
        _ASSERTE( m_hFile != INVALID_HANDLE_VALUE );
        return ::GetFileTime( m_hFile, ftCreate, ftAccess, ftModified );
    }

    BOOL SetFileTime( FILETIME * ftCreate, FILETIME * ftAccess,
                      FILETIME * ftModified ) {
        _ASSERTE( m_hFile != INVALID_HANDLE_VALUE );
    ...
    BOOL DuplicateHandle( HANDLE hOther ) {
        ...
        return ::DuplicateHandle( process, hOther, process, &m_hFile, NULL, FALSE,
    ...
    static BOOL FileExists( LPCTSTR pstrFileName ) {
        ...
        return dwAttribs != INVALID_FILE_ATTRIBUTES &&
               ( ( dwAttribs & FILE_ATTRIBUTE_DIRECTORY ) == 0 );
    }
